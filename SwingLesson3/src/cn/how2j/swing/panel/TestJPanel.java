...
/**
 * JPanel即为基本面板
 * JPanel和JFrame一样都是容器，不过面板一般用来充当中间容器，把组件放在面板上，然后再把面板放在窗体上JFrame。
 * 一旦移动一个面板，其上面的组件，就会全部统一跟着移动，采用这种方式，便于进行整体界面的设计 
 *
 *	JPanel默认就是采用的FlowLayout布局！
 */
public class TestJPanel {
	public static void main(String[] args) {
        ...
        p1.setBounds(50, 50, 300, 60);// 设置面板大小
        p1.setBackground(Color.RED);// 设置面板背景颜色
        p1.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
        p1.setLayout(new FlowLayout());// 这一句可以没有，因为JPanel默认就是采用的FlowLayout
